    ...
    public class XmlDocumentationLoader
    {
        ...
        public void AddDeclarations(TableDeclarationProvider tableDeclarationProvider)
        {
            ...
            foreach (XElement doc in docs)
            {
                ...
                foreach (XElement table in doc.XPathSelectElements("./tables/table"))
                {
                    AddTableDeclaration(tableDeclarationProvider, doc, table);
                }

                if (doc.Element("globals") != null)
                {
                    // Query the documentation for global declarations and add them to the global table
                    foreach (XElement element in doc.Element("globals").Elements())
                    {
                        AddDeclaration(tableDeclarationProvider, TableDeclarationProvider.DeclarationsTable, element);
                    }
                }
            }
        }
        ...
        private void AddTableDeclaration(TableDeclarationProvider tableDeclarationProvider, XElement doc, XElement table)
        {
            ...
            if (inherits != null)
            {
                ...
                foreach (string inheritValue in inheritValues)
                {
                    ...
                    if (baseTable != null)
                    {
                        foreach (XElement element in baseTable.Elements())
                        {
                            AddDeclaration(tableDeclarationProvider, name, element);
                        }
                    }
                }
            }
            ...
            {
                AddDeclaration(tableDeclarationProvider, name, element, src);
            }
        }
